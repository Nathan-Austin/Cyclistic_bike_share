...
Lets drop the rides of an hour or less
...
```{r}
ggplot(long_hire_filtered, aes(x = user_type, y = numberofrides, fill = longhire)) +
  geom_bar(stat = "identity", position = "dodge") +
  labs(title = "Count of each longhire and group them by user_type",
       x = "User Type",
       y = "Number of Rides",
       fill = "Long Hire") +
  theme_minimal() +
...
Lets have a closer look at the multiday hires

```{r}
long_hire_filtered1 <- long_hire %>% 
  filter(longhire %in% c("1-2 days", "2-4 days", "5 days or more"))

ggplot(long_hire_filtered1, aes(x = user_type, y = numberofrides, fill = longhire)) +
  geom_bar(stat = "identity", position = "dodge") +
  labs(title = "Count of each longhire and group them by user_type",
       x = "User Type",
       y = "Number of Rides",
...
From and inital 4299967 ride we can see a total of 131 rides that went for more than a day. This number is insignificant in the scheme of thing and could be from user error when closing out the ride or from errors in the docking station so no further investigation is warranted.
